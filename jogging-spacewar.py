...
# 載入圖片
background_img = pygame.image.load(os.path.join("Resource/img", "background.png")).convert()
background_img = pygame.transform.scale(background_img, (WIDTH, HEIGHT))
# background_img = pygame.image.load(os.path.join(custom, "1-1.jpg")).convert()
player_img = pygame.image.load(os.path.join("Resource/img", "player.png")).convert()
bullet_img = pygame.image.load(os.path.join("Resource/img", "bullet.png")).convert()
rock_imgs = []
for i in range(7):
    rock_imgs.append(pygame.image.load(os.path.join("Resource/img", f"rock{i}.png")).convert())
font_name = os.path.join("Resource/font.ttf")
expl_anim = {}
expl_anim['lg'] = []
expl_anim['sm'] = []
for i in range(9):
    expl_img = pygame.image.load(os.path.join("Resource/img", f"expl{i}.png")).convert()
...
timeboard_img = pygame.image.load(os.path.join("Resource/img", "timeboard.png")).convert()
...
def draw_init():
    ...
    draw_text(screen, '戰鬥機-超慢跑', 64, WIDTH / 2, HEIGHT / 4)
...
while running:
    ...
    if results.pose_landmarks:
        ...
        left_ankle = landmarks[mpPose.PoseLandmark.LEFT_ANKLE].y
        right_ankle = landmarks[mpPose.PoseLandmark.RIGHT_ANKLE].y

        # 記錄 Y 軸變化
        left_ankle_y.append(left_ankle)
        right_ankle_y.append(right_ankle)
        ...
        # 計算變化幅度
        left_variation = max(left_ankle_y) - min(left_ankle_y)
        right_variation = max(right_ankle_y) - min(right_ankle_y)

        def isMoving():
            # 設定閾值 (threshold)，大於 0.03 就視為運動
            threshold = 0.03
            if left_variation > threshold or right_variation > threshold:
                return True
            else:
                return False
    ...
    if(isMoving()):
        ...
        player_stats = "Standing"
    draw_text(screen, player_stats, 60, WIDTH/2, 50)
